...
public class OmsOrderReturnReasonController extends BaseController
{
     ...
     * 查询退货原因列表
     */
    @PreAuthorize("@ss.hasPermi('order:returnReason:list')")
    @GetMapping("/list")
     ...
     * 导出退货原因列表
     */
    @PreAuthorize("@ss.hasPermi('order:returnReason:export')")
     ...
     * 获取退货原因详细信息
     */
    @PreAuthorize("@ss.hasPermi('order:returnReason:query')")
    @GetMapping(value = "/{id}")
     ...
     * 新增退货原因
     */
    @PreAuthorize("@ss.hasPermi('order:returnReason:add')")
     ...
     * 修改退货原因
     */
    @PreAuthorize("@ss.hasPermi('order:returnReason:edit')")
     ...
     * 删除退货原因
     */
    @PreAuthorize("@ss.hasPermi('order:returnReason:remove')")
